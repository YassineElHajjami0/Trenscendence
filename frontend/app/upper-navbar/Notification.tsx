...
function Notification({ notif }: { notif: any }) {
  ...
  const handleJoin = async (uid: number, channelId: number) => {
    await fetch(
      `http://10.13.4.4:3000/channelss/joinpublic?userID=${uid}&channelID=${channelId}`,
      {
        method: "PATCH",
        headers: {
  ...
  const deleteNotificatio = async () => {
    try {
      await fetch(`http://10.13.4.4:3000/notifications/${notif?.id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${userTok}`,
          "Content-Type": "application/json",
  ...
  const acceptFriend = async () => {
    ...
    try {
      await fetch(`http://10.13.4.4:3000/friends`, {
        method: "POST",
