...
const PopupSearchChannels: React.FC<popupProps> = ({
  ...
  const handleJoin = async (id: number) => {
    const req = await fetch(
      `http://10.13.4.8:3000/channelss/roles?channelId=${id}`,
      {
        headers: {
    ...
    await fetch(
      `http://10.13.4.8:3000/channelss/joinpublic?userID=${userId}&channelID=${id}`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${userTok}`,
  ...
  useEffect(() => {
    ...
    const fetchFreshChannels = async () => {
      const response = await fetch(
        `http://10.13.4.8:3000/channelss?mustinclude=${channelName}`,
        {
          headers: {
            Authorization: `Bearer ${userTok}`,
            "Content-Type": "application/json",
