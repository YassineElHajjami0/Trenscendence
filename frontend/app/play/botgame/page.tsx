...
function RobotGame() {
	...
	const ball = new Ball(CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, BALL_RADIUS, BALL_DX, BALL_DY, BALL_SPEED);
	const leftPaddle = new Paddle(0, CANVAS_HEIGHT / 2 - PADDLE_HEIGHT / 2, PADDLE_WIDTH, PADDLE_HEIGHT);
	const rightPaddle = new Paddle(CANVAS_WIDTH - PADDLE_WIDTH, CANVAS_HEIGHT / 2 - PADDLE_HEIGHT / 2, PADDLE_WIDTH, PADDLE_HEIGHT);
	...
	useEffect(() => {
		if (botPaddleSpeed) {
			const controlButtons = document.querySelector('.control-buttons') as HTMLDivElement;
	
			window.addEventListener('resize', () => {
				if (window.innerWidth < 769) {controlButtons.style.display = 'flex'};
			});
	
			window.addEventListener('resize', () => {
				if (window.innerWidth > 769) {controlButtons.style.display = 'none'};
			});
	...
	useEffect(() => {
		...
		const handleStartGame = () => {
			...
			const resignButton = document.getElementById('resign-button') as HTMLSpanElement;
			const message = document.getElementById('msg') as HTMLDivElement;
			const buttons = document.querySelector('.level-button-container') as HTMLDivElement;
			const quitButton = document.querySelector('.quit-game-button') as HTMLButtonElement;
		...
		const drawCircle = (x: number, y: number, r: number, color: string | CanvasGradient | CanvasPattern) => {
			ctx.fillStyle = color;
			ctx.beginPath();
		...
		const drawRect = (x: number, y: number, w: number, h: number, color: string | CanvasGradient | CanvasPattern) => {
			ctx.fillStyle = color;
		...
		const collision = (b: { x: number, y: number, raduis: number }, p: { x: number, y: number, width: number, height: number }) => {
		...
		const update = (ball: Ball, leftPaddle: Paddle, rightPaddle: Paddle) => {
			if (ball.x < -ball.raduis) {
				...
				if (rightPaddle.score === WINNER_SCORE) {
					...
					message.innerHTML = 'You lost the game.<br /> going back to play page in <br />3 seconds...';
				...
				if (leftPaddle.score === WINNER_SCORE) {
					...
					message.innerHTML = 'You won the game.<br /> going back to play page in <br />3 seconds...';
			...
			} else if ((keyState['ArrowDown'] || keyState['right']) && leftPaddle.y + leftPaddle.height < canvas!.height) {
				leftPaddle.y += PADDLE_SPEED;
			}
			...
			} else if (rightPaddle.y + rightPaddle.height / 2 > ball.y + ball.raduis) {
				rightPaddle.y -= botPaddleSpeed;
			}
		...
		<div className='game-container'>
			<div className="canvas-container">
				<button onClick={() => setResign(true)} id='resign-button'>
					{'Resign'}
					<Img priority={true} src="/game/resign.png" alt="" width={20} height={20} />
				</button> 
				<div className="control-buttons">
					<button id='left' className='control-button' ref={leftArrowRef}>{"<"}</button>
					<button id='right' className='control-button' ref={rightArrowRef}>{">"}</button>
				</div>
				...
				<button className='quit-game-button' onClick={handleQuit}>Quit</button>
				<div className="info-bar">
					<div className="left-player-info">
						<Img src={avatar ? avatar : defaultAvatar.src} alt="" width={50} height={50} />
						<span>{username}</span>
						<Img priority={true} id='faceLeft' src={leftFace} alt="" width={30} height={30} />
					</div>
					...
					<div className="right-player-info">
						<Img priority={true} id='faceRight' src={rightFace} alt="" width={30} height={30} />
