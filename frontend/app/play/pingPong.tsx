...
export class Ball {
	...
	constructor(x: number, y: number, raduis: number, dx: number, dy: number, speed: number) {
		this.x = x;
		this.y = y;
		this.raduis = raduis;
		this.dx = dx;
...
export function PingPong() {
	...
	useEffect(() => {
		...
		if (gameMode === 'random') {
			socket.on('start_game',( payBall, paddle1, paddle2, {userId, opponentId}) => {
				randerGame(payBall, paddle1, paddle2);
				fetchUserDatas(userId, opponentId);
				setGameStarted(true);
			});
		} else if (gameMode === 'friend') {
			socket.on('start_friend_game',( payBall, paddle1, paddle2, {userId, opponentId} ) => {
				randerGame(payBall, paddle1, paddle2);
				fetchUserDatas(userId, opponentId);
				setGameStarted(true);
			});
		...
		socket.on('update', (ball: Ball, leftPaddle: Paddle, rightPaddle: Paddle) => {
		...
		function drawCircle(x: number, y: number, r: number, color: string | CanvasGradient | CanvasPattern) {
			ctx.fillStyle = color;
			ctx.beginPath();
		...
		<div className='game-container'>
			...
			<div className="canvas-container"> 
				...
				<button id='resign-button' onClick={resign}>
					{'Resign'}
					<Img priority={false} src="/game/resign.png" alt="" width={20} height={20} />
				</button>
				<div className='overlay'></div>
				<div id="msg"></div>
				<button className='start-game-button' onClick={
					gameMode === 'random' ? joinQueue : readyToStart
				}>{buttonText}</button>
				<button className='quit-game-button' onClick={Quit}>Quit</button>
				<div className="info-bar">
					<div className="left-player-info">
						<Img priority={false} id='' src={leftAvatar ? leftAvatar : defaultAvatar} alt="" width={50} height={50} />
						<span>{leftPlayer}</span>
						<Img priority={false} id='faceLeft' src={leftFace} alt="" width={30} height={30} />
					</div>
					...
					<div className="right-player-info">
						<Img priority={false} id='faceRight' src={rightFace} alt="" width={30} height={30} />
						<span>{rightPlayer}</span>
						<Img priority={false} id='' src={rightAvatar ? rightAvatar : defaultAvatar} alt="" width={50} height={50} />
					</div>
				</div>
