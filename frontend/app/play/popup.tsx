...
export default function Popup({ setShowPopup }: any) {
	const [loadingStates, setLoadingStates] = useState<{ [index: number]: boolean }>({});
	...
	const onlineFriends = allUsers.filter((user: any) => user.status === 'online');
	...
	const sendGameReq = (opponentId: number, index: number) => {
		...
		socket.emit('game_request', { userId: userId, opponentId: opponentId, index: index, table: table});
