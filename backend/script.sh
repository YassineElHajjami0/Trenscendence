...
check_migrations() {
    PGPASSWORD="$POSTGRES_PASSWORD" psql -h postgres -U "$POSTGRES_USER" -d "$POSTGRES_DB" -t -c "SELECT COUNT(*) FROM _prisma_migrations" | tr -d '[:space:]'
}
...
if [ "$migration_count" -gt "0" ]; then
  ...
  INSERT INTO "Item" (img , name , description , price , type , power , color)
  VALUES ('$BACK_URL/pd/Crimson Reaper.png', 'Crimson Reaper', 'An intense red paddle embodying power and control, perfect for players with relentless determination and aggression.', 700, 'paddle', 'Power', 'green');
  INSERT INTO "Item" (img , name , description , price , type , power , color)
  VALUES ('$BACK_URL/pd/Neon Viper.png', 'Neon Viper', 'A vibrant green paddle known for its speed and accuracy, ideal for players who thrive on the edge.', 100, 'paddle', 'Power', 'pink');
  INSERT INTO "Item" (img , name , description , price , type , power , color)
